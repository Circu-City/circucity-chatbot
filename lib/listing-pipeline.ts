...
// The listing analysis pipeline shared by the sandbox and the authenticated
// product. Kept in one place so the AI behaviour cannot drift between the test
// desk and the real app.
...
const analysisSchema = z.object({
  ...
  // Stock available — default 1 for unique second-hand items; more only when the
  // seller is clearly listing several identical units (e.g. a bundle of 5 chairs).
  quantity: z.coerce.number().int().min(1).max(9999).optional().default(1),
  // Gemini sometimes flattens "attributes" into one "Key: value, Key2: value2"
  // string. Normalize before validating so a well-formed result isn't lost.
...
const categoryMedians: Record<string, number> = {
  "sustainable fashion": 350, "bag": 250, "eco home": 450,
  "electronics": 1200, "general": 250, "green gadgets": 900,
  "recycled items": 200, "skincare": 180,
};

const categoryWeightsKg: Record<string, number> = {
  "sustainable fashion": 0.5, "bag": 0.9, "eco home": 3,
  "electronics": 1.5, "general": 0.5, "green gadgets": 0.6,
  "recycled items": 1, "skincare": 0.3,
};

const categoryCo2PerKg: Record<string, number> = {
  "sustainable fashion": 25, "bag": 25, "eco home": 15,
  "electronics": 8, "general": 12, "green gadgets": 8,
  "recycled items": 5, "skincare": 5,
};

const conditionMultipliers = { new: 1, like_new: 0.85, good: 0.65, fair: 0.45, poor: 0.25 };

const CATALOGUE_INSTRUCTIONS = `You catalogue second-hand marketplace products for the CircuCity Swedish resale marketplace.
Reply with ONLY a JSON object with keys: category, title, description, condition, suggested_price_sek, estimated_age, estimated_weight_kg, quantity, attributes, grounded.
category MUST be one (and only one) of these marketplace categories: ${MARKETPLACE_CATEGORIES.join(", ")}.
If the item is a handbag, backpack, tote, satchel, clutch, wallet, duffel, briefcase or any kind of bag put it in "Bag" — never put bags in "General" or in "Sustainable Fashion".
If unsure, put it in "General" and give a fallback price estimate rather than guessing a wrong category.
Condition must be one of new, like_new, good, fair, poor. Title max 80 characters; description max 300 characters.
quantity is the number of identical units the seller has in stock — use 1 unless the photo clearly shows several identical units being sold together.
Be honest about visible wear. Never invent a brand, model, material, size, age, authenticity, or functionality that is not clearly visible or confirmed.
estimated_weight_kg is the typical shipping weight in kilograms for an item like this (e.g. a t-shirt is about 0.2, a handbag about 0.9, a laptop about 1.8, a book about 0.3).
estimated_age must be a short string (e.g. "2 years", "Unknown"), not a number.
...
export async function analyzeListingImage(body: { imageDataUrl?: string; titleHint?: string; language?: string }): Promise<ListingAnalysisResult> {
  ...
  if (parsed.data.imageDataUrl && !/^data:image\/(jpeg|jpg|png|webp);base64,/i.test(parsed.data.imageDataUrl)) {
    return { ok: false, error: "AI analysis supports JPEG, PNG, and WEBP images", status: 400 };
  }
...
// Small helper so callers that need a NextResponse stay consistent.
