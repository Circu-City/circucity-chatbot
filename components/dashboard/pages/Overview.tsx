...
export default function Overview() {
  ...
  const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
    idle: { label: "Waiting", color: "bg-yellow-100 text-yellow-700 border-yellow-200", icon: Clock },
    crawling: { label: "Syncing...", color: "bg-blue-100 text-blue-700 border-blue-200", icon: RefreshCw },
    completed: { label: "Synced", color: "bg-green-100 text-green-700 border-green-200", icon: CheckCircle2 },
    failed: { label: "Sync Failed", color: "bg-red-100 text-red-700 border-red-200", icon: XCircle },
  };
    ...
    <Wrapper>
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center">
          <p className="text-muted-foreground mb-2">No store data found.</p>
        ...
        <>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div>
                  <p className="text-sm font-semibold text-dark-navy">
                    {trialDaysLeft <= 0 ? "Your trial has ended" : "Your trial ends in " + trialDaysLeft + " day" + (trialDaysLeft === 1 ? "" : "s")}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {trialDaysLeft <= 0 ? "Upgrade to keep your chatbot running and access all features." : "Upgrade before your trial ends to keep your chatbot running without interruption."}
                  </p>
                </div>
              </div>
              <Button variant="primary" size="sm" className="shrink-0" onClick={() => router.push("/dashboard?tab=billing")}>
                {trialDaysLeft <= 0 ? "Reactivate" : "Upgrade Now"}
              </Button>
            </div>
          )}

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard title="Messages Today" value={stats?.totalMessages?.toLocaleString() || "0"} change={stats?.conversationsThisMonth ? "+" + stats.conversationsThisMonth : "0"} icon={MessageSquare} trend="up" />
            <StatCard title="Conversion Rate" value={(stats?.conversionRate || "0") + "%"} change={(stats?.resolutionRate || "0") + "% resolved"} icon={TrendingUp} trend="up" />
            <StatCard title="Total Conversations" value={stats?.totalConversations?.toLocaleString() || "0"} change={(stats?.resolvedCount || 0) + " resolved"} icon={Users} trend={stats?.resolvedCount > 0 ? "up" : "down"} />
            <StatCard title="Resolution Rate" value={(stats?.resolutionRate || "0") + "%"} change={(stats?.csatScore || "0") + "/5 CSAT"} icon={ShoppingBag} trend="up" />
          </div>
          ...
          <Card className="border-border shadow-sm overflow-hidden">
            <div className="p-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-dark-navy">Website Ownership</h3>
                  <p className="text-xs text-muted-foreground">
                    {syncStatus?.ownershipVerified ? "Your website ownership is verified." : "Verify your website by installing the widget code to enable crawling."}
                  </p>
                </div>
              </div>
              <Badge className={cn("px-3 py-1", syncStatus?.ownershipVerified ? "bg-green-100 text-green-700 border-green-200" : "bg-yellow-100 text-yellow-700 border-yellow-200")}>
                {syncStatus?.ownershipVerified ? "Verified" : "Not Verified"}
              </Badge>
            </div>
            {!syncStatus?.ownershipVerified && (
              <div className="px-6 pb-6">
                <Button variant="primary" size="sm" onClick={() => router.push("/dashboard?tab=widget")}>Install Widget Code</Button>
              </div>
            )}
          </Card>
          ...
          <Card className="border-border shadow-sm overflow-hidden">
            <div className="p-6 border-b border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-dark-navy">Website Sync Status</h3>
                  <p className="text-xs text-muted-foreground">
                    {syncStatus?.ownershipVerified ? "Your chatbot crawls your website every 6 hours." : "Install the widget code to enable crawling."}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-medium", syncStatus?.ownershipVerified ? sc.color : "bg-yellow-100 text-yellow-700 border-yellow-200")}>
                  ...
                  {syncStatus?.ownershipVerified ? sc.label : "Not Verified"}
                </div>
                {syncStatus?.ownershipVerified && (
            ...
            <div className="p-6">
              {!syncStatus?.ownershipVerified ? (
                <div className="flex flex-col items-center py-6 text-center">
                  ...
                  <h4 className="font-bold text-dark-navy mb-1">Website Not Verified</h4>
                  <p className="text-sm text-muted-foreground mb-4 max-w-md">
                    Your website ownership hasn't been verified yet. Paste the widget code into your website to automatically verify.
                  </p>
                  <Button variant="primary" size="sm" onClick={() => router.push("/dashboard?tab=widget")}>Go to Widget Configuration</Button>
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div><p className="text-xs text-muted-foreground mb-1">Pages Crawled</p><p className="text-lg font-bold text-dark-navy">{syncStatus?.pagesCrawled || 0}<span className="text-xs text-muted-foreground font-normal ml-1">pages</span></p></div>
                    <div><p className="text-xs text-muted-foreground mb-1">Products Synced</p><p className="text-lg font-bold text-dark-navy">{syncStatus?.productsSynced || 0}<span className="text-xs text-muted-foreground font-normal ml-1">products</span></p></div>
                    <div><p className="text-xs text-muted-foreground mb-1">Last Sync</p><p className="text-lg font-bold text-dark-navy">{syncStatus?.lastCrawl ? formatTimeAgo(syncStatus.lastCrawl) : "Never"}</p></div>
                    <div><p className="text-xs text-muted-foreground mb-1">Next Sync</p><p className="text-lg font-bold text-dark-navy">{syncStatus?.nextCrawl ? formatTimeAgo(syncStatus.nextCrawl) : "—"}</p></div>
                  </div>
                  ...
                  {syncStatus?.status === "failed" && (
                    <div className="mt-4 pt-4 border-t border-border flex items-center gap-2 text-xs">
                      ...
                      <span className="text-red-600">Sync failed. Your chatbot might not have the latest data.</span>
                    </div>
                  )}
                  {syncStatus?.status === "idle" && !syncStatus?.lastCrawl && (
                    <div className="mt-4 pt-4 border-t border-border flex items-center gap-2 text-xs">
                      <AlertCircle className="w-3.5 h-3.5 text-yellow-500" />
                      <span className="text-yellow-600">Your website hasn't been synced yet. Click <strong>Sync Now</strong> to start the initial crawl.</span>
                    </div>
                  )}
                </>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="lg:col-span-2 border-border shadow-sm overflow-hidden">
              <div className="p-6 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-2 font-bold text-dark-navy"><Clock className="w-5 h-5 text-primary" /> Recent Conversations</div>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-muted/50 text-[10px] uppercase tracking-widest text-muted-foreground font-bold">
                    <tr><th className="px-6 py-3">Customer</th><th className="px-6 py-3">Status</th><th className="px-6 py-3 text-right">Date</th></tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {conversations.length > 0 ? conversations.map((conv: any) => (
                      <tr key={conv.id} className="hover:bg-muted/30 transition-colors group cursor-pointer">
                        ...
                        <td className="px-6 py-4"><Badge variant={conv.resolved ? "primary" : "outline"} className="text-[10px]">{conv.resolved ? "Resolved" : "Active"}</Badge></td>
                        <td className="px-6 py-4 text-sm text-muted-foreground text-right">{new Date(conv.createdAt).toLocaleDateString()}</td>
                      </tr>
                    )) : (
                      <tr><td colSpan={3} className="px-6 py-12 text-center text-sm text-muted-foreground">No conversations yet</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div className="space-y-4">
              <Card className="border-border shadow-sm bg-dark-navy text-white p-6 relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <h3 className="font-bold text-lg mb-2">Your Store</h3>
                  <p className="text-sm text-gray-400 mb-4">{store?.name} — {syncStatus?.productsSynced || 0} synced products</p>
                  <Button variant="primary" size="sm" className="w-full" onClick={() => router.push("/dashboard?tab=widget")}>Configure Widget</Button>
                </div>
              ...
              <Card className="border-border shadow-sm p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-xs"><span className="text-muted-foreground">Plan</span><span className="text-dark-navy font-medium capitalize">{store?.subscriptions?.[0]?.plan || "Free"}</span></div>
                  <div className="flex justify-between text-xs pt-2"><span className="text-muted-foreground">Status</span><span className="text-dark-navy font-medium capitalize">{store?.subscriptions?.[0]?.status || "Active"}</span></div>
                </div>
              </Card>
            </div>
          </div>
