  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setAddError(json?.error || "Could not add the product. Please try again.");
        return;
      }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-border">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-dark-navy mb-1">Product name *</label>
                  <Input value={addForm.name} onChange={(e) => setAddForm({ ...addForm, name: e.target.value })} placeholder="e.g. Vintage denim jacket" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-dark-navy mb-1">Price *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-dark-navy mb-1">Currency</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-dark-navy mb-1">Category</label>
                  <Input value={addForm.category} onChange={(e) => setAddForm({ ...addForm, category: e.target.value })} placeholder="e.g. Clothing" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-dark-navy mb-1">Stock</label>
                  <Input type="number" min="0" value={addForm.stock} onChange={(e) => setAddForm({ ...addForm, stock: e.target.value })} placeholder="0" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-dark-navy mb-1">Description</label>
                <textarea
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
              <Button variant="outline" size="sm" onClick={() => setShowAddModal(false)} disabled={saving}>Cancel</Button>
